...
export async function deleteNote(req,res){
    try{
        const {title,content}=req.body;
        const checkdelete=await Note.findByIdAndDelete(req.params.id);
        if(checkdelete) return res.status(200).json({message:"Note deleted successfully"});
        ...
        if(notetofind)
            {
                ...
                return res.status(200).json({message: "Note found ",notetofind});
            } 
        else return res.status(404).json({message:"Note Not Found"})
    } catch(error) {
